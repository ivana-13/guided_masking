...
class Conceptual_Caption(td.RNGDataFlow):
    ...
    def __init__(self, corpus_path, shuffle=False):
        ...
        self.name = os.path.join(corpus_path, 'resnet101_faster_rcnn_genome_imgfeats/val2014_obj36-36.tsv')
        self.infiles = [self.name]
        self.counts = []

        self.captions = {}
        df = json.load(open(os.path.join(corpus_path, 'annotations/test_ann.jsonl')))
        ...
        df = json.load(open(os.path.join(corpus_path, 'annotations/valid_ann.jsonl')))
        ...
        with open(os.path.join(corpus_path, 'annotations/test_ann.jsonl')) as f:
            captions = json.load(f)["annotations"]
        with open(os.path.join(corpus_path, 'annotations/valid_ann.jsonl')) as f:
            captions2 = json.load(f)["annotations"]
        with open(os.path.join(corpus_path, 'annotations/train_ann.jsonl')) as f:
            captions3 = json.load(f)["annotations"]

        self.num_caps = (len(captions)+len(captions2)+len(captions3))
...
ds1 = td.MultiProcessRunner(ds, 2000, 1)
td.LMDBSerializer.save(ds1, os.path.join(corpus_path, 'imgfeats/volta/validation_feat_all.lmdb'), write_frequency=2000)

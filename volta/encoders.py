...
class BertGatedSelfAttention(nn.Module):
    def __init__(self, config, layer_num):
        ...
        self.has_text = (self.has_tt or self.has_tv or self.has_vt)
        self.has_vision = (self.has_vv or self.has_vt or self.has_tv)
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self, input_txt, input_imgs, image_loc, token_type_ids=None, attention_mask=None,
                image_attention_mask=None, task_ids = None, output_all_encoded_layers=False, output_all_attention_masks=False):
...
class BertForVLPreTraining(BertPreTrainedModel):
    ...
    def forward(
        ...
        prediction_scores_t, prediction_scores_v, seq_relationship_score, vqa_score, pooled_output = self.cls(
        ...
        # img_loss = self.vis_criterion(F.log_softmax(prediction_scores_v, dim=2), image_cls)
        # masked_img_loss = torch.sum( img_loss * (image_label == 1).unsqueeze(2).float()) / max(torch.sum((image_label == 1)), 1)
        ...
        # next_sentence_loss = next_sentence_loss.unsqueeze(0)
        # vqa_score = None
...
class BertForVLTasks(BertPreTrainedModel):
    def __init__(self, config, task_cfg, task_ids, dropout_prob=0.1):
        ...
        for task_id in task_ids:
            ...
            elif task_type.startswith("word-prediction"):
                task2clf[task_id] = nn.Linear(config.pooler_size, 1)
            elif task_type.startswith("token-prediction"):
                task2clf[task_id] = nn.Linear(config.pooler_size, 1)
            else:
    ...
    def forward(
        ...
        linguistic_prediction, vision_prediction = pooled_output_t, sequence_output_v
        ...
        if self.task_cfg[task_id]["type"].startswith("V-logit"):
            ...
            # NLVR
            vil_prediction = seq_relationship_score
        else:
        ...
        return vil_prediction, vision_prediction, linguistic_prediction, all_attention_mask, prediction_scores_t.view(-1, self.config.vocab_size)
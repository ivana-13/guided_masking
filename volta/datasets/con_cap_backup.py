...
class ConceptCapLoaderTrain(object):
    ...
    def __init__(
        ...
        if dist.is_available() and local_rank != -1:
            ...
            lmdb_file = os.path.join(corpus_path, "COCO_trainval_resnext152_faster_rcnn_genome.lmdb/data.mdb")
        ...
        caption_path = "datasets/mscoco/annotations/train_ann.jsonl"
        # caption_path = os.path.join(corpus_path, "caption_val.json")
        self.num_dataset = len(json.load(open(caption_path, "r"))['annotations'])
...
class ConceptCapLoaderVal(object):
    ...
    def __init__(
        ...
        lmdb_file = os.path.join(corpus_path, "COCO_test_resnext152_faster_rcnn_genome.lmdb/data.mdb")
        caption_path =  "datasets/mscoco/annotations/valid_ann.jsonl"
        print("Loading from %s" % lmdb_file)
...
class BertPreprocessBatch(object):
    def __init__(
        ...
        self.captions = json.load(open(caption_path, "r"))['annotations']
        self.visualization = visualization
        self.objective = objective
        self.bert_model = bert_model

        self.caption_dictionary = {}
        df = json.load(open(caption_path, "r"))

        for annotation in df["annotations"]:
            captions = annotation['sentences']
            image_id = annotation['id']
            self.caption_dictionary[image_id] = captions

    def __call__(self, data):

        image_feature_wp, image_target_wp, image_location_wp, num_boxes, image_h, image_w, image_id = (
            data
        )

        print(image_feature_wp)

        print(image_target_wp)
        
        print(image_location_wp)
    ...
    def get_random_caption(self):
        ...
        caption = [self.captions[rand_doc_idx]['sentences'][idx]]
        return caption

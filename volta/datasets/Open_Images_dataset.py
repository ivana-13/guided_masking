...
def _load_annotations(annotations_jsonpath, negative_pairs_jsonpath):
    ...
    entries = []
    for pair in annotations_json.keys():
        if list(annotations_json[pair].keys())[0] != pair:
            ...
            entries.append(
            {   
                "caption": annotations_json[str(pair)][pair][0].lower(),
                "foil": 0,
                "image_id": pair,
                "caption_number": pair,
                "caption_verb": annotations_json[str(pair)][pair][1].lower(),
                "correct_verb": annotations_json[str(pair)][pair][1].lower(),
                "caption_subject": annotations_json[str(pair)][pair][3].lower(),
            }
            )

    return entries

class Open_Images_ProbesClassificationDataset(Dataset):
    def __init__(
        self,
        task,
        dataroot,
        ...
        for entry in self._entries:
            ...
            if entry["foil"] == 0:
                ...
                for i in range(len(result)):
                    if ("root" in result[i]['deprel']) and ((result[i]['upos']=='NOUN') or (result[i]['upos']=='PROPN')):
                        if subj == []:
                            subj = result[i]['text']
                for i in range(len(result)):
                    if ("subj" in result[i]['deprel']) and ((result[i]['upos']=='NOUN') or (result[i]['upos']=='PRON')or (result[i]['upos']=='PROPN')):
                        if subj == []:
                            subj = result[i]['text']
                for i in range(len(result)):
                    if ("compound" in result[i]['deprel']) and (result[i]['upos']=='NOUN'):
                        if subj == []:
                            subj = result[i]['text']
                for i in range(len(result)):
                    if (("obj" in result[i]['deprel']) or ("obl" in result[i]['deprel'])) and (result[i]['upos']=='NOUN'):
                        if subj == []:
                            subj = result[i]['text']
                for i in range(len(result)):
                    if ("nmod" in result[i]['deprel']) and (result[i]['upos']=='NOUN'):
                        if subj == []:
                            subj = result[i]['text']
                if subj == []:
                    print('no subj')
                ...
                verb_in_caption = [word for word in (str(entry["caption"])).replace(',','').split() if lemmatizer.lemmatize(word, pos="v").find(entry["caption_verb"]) != -1]
                if verb_in_caption[-3:] == 'ing':
                    entry["correct_verb"] = entry["correct_verb"] + 'ing'
                elif verb_in_caption[-1] == 's':
                    entry["correct_verb"] = entry["correct_verb"] + 's'
                elif verb_in_caption[-2:] == 'ed':
                    entry["correct_verb"] = entry["correct_verb"] + 'ed'
                text = []
                caption_words = (str(entry['caption'])).split()

                for i in range(len(caption_words)):
                    if caption_words[i] in verb_in_caption:
                        text += [entry["correct_verb"]]
                        verb_in_caption = [entry["correct_verb"]]
                    else:
                        text += [caption_words[i]]
                tokens = self._tokenizer.encode(' '.join(text))
                # tokens = self._tokenizer.encode(entry["caption"])
    ...
    def __getitem__(self, index):
        ...
        boxes_reshaped = boxes.reshape((37,5))[:36,:4]
        ...
        if self.vision_masking == 'partial':
            if entry['subj'] in list(self.object_vocab):
                ...
                if list(self.object_vocab).index(entry['subj']) in obj_labels_backround:
                    ...
                    try:
                        if len(entry['subj'])>1:
                            entry['subj'] = entry['subj'].replace(' ', '_')
                ...
                try:
                    if entry['caption_subject'] != '':
                        entry['subj'] = entry['caption_subject']
                    if entry['subj'] == 'up':
                        entry['subj'] = 'bicycle'
                    caption_subj = wordnet.synsets(entry['subj'])[0]
                except:
            ...
            if count == 0:
                print('count 0')
                print(entry['subj'])
                print(self.object_vocab[list_of_subjects])

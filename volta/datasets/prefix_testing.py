...
class PrefixPredictionDataset(Dataset):
    def __init__(
        self,
        task,
        dataroot,
        ...
        for entry in self._entries:
            ...
            if self.task.split('_')[-1] == "masking":
                if entry["foil"] == 0:
                    ...
                    prefix = ''
                    print(entry['caption'])
                    for word in entry['caption'].split():
                        print(word)
                        prefix += word + " "
                        if word == '[MASK]':
                            break
                    entry['caption'] = prefix
            ...
            if self.task.split('_')[-1] == "masking":
                ...
                for i, token in enumerate(tokens):
                    if token == self._tokenizer.convert_tokens_to_ids(self._tokenizer.mask_token):
                        output_label.append(verb_tokens[0])
                        break
                    else:
            ...
            if len(tokens) < self._max_seq_length:
                ...
                segment_ids += padding
                if self.task.split('_')[-1] == "masking":
                    padding = [self._padding_index] * (self._max_seq_length - len(output_label))
